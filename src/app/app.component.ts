...
export class AppComponent {
  ...
  public image : string = "Breaking Bad";
  public description : string = "Breaking Bad"; 
  public show : Show = {
    name: "Breaking Bad",
    image: 'Breaking Bad',
    ...
    @Output()
    public chargeData : EventEmitter<Show> = new EventEmitter();
  ...
  public updateElement(show: Show): void {
    const index = this.shows.findIndex(oldShow => oldShow.name === this.oldShow.name);
    if (index !== -1){
  ...
  public createElement(show : Show): void{
    ...
    if(this.form.valid){
        const newShow: Show = {
            ...
            likes: [],
            name: this.form.value.description,
            year: 0
        }
        this.shows.push(newShow);
        this.form.reset();
